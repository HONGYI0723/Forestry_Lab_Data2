...
filepath <-'C:/Users/谢弘逸/Downloads/test/ForL2/files/'
...
#Visualize dominant species distribution across aspect categories.
ttm()
tm_shape(asp, alpha = 0.5) +
 ...
 #Visualize dominant species distribution across slope.
ttm()
tm_shape(slope, alpha = 0.5) +
  tm_raster(style = "cont", alpha = 0.6, title = "Slope (deg)") +
  tm_shape(sf_plot) +
...
# Visualize BA values with a topographic map background.
ttm()
tm_shape(sf_plot) +
